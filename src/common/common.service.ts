...
import { BasePaginationDto } from '../posts/dto/base-pagination.dto';
import {
  FindManyOptions,
  FindOptionsOrder,
  FindOptionsWhere,
...
export class CommonService {
  ...
  private composeFindOptions<T extends BaseModel>(
     ...
     * }
     * 현재 where__id__more_than / where__id__less_than에 해당되는 where 필터만 사용중이지만
  ...
  private parseWhereFilter<T extends BaseModel>(
    ...
    if (split.length === 2) {
       ...
       * where는 버려도 되고 두번재 값은 필터할 키값이 되고
       * 세번째 값은 typeorm 유틸리티가 된다.
       *
       * FILTER_MAPPER 미리 정의해둔 값들로
